...
feature/v1.1 分支是基于 feature/v1 创建的，目前两个分支都分别提交了一些东西，我们希望将 feature/1.1 的最后一个提交（5:288d0414598d）复制到 feature/1 中，可以这样：
...
如果只是修改最后一个 commit 信息，我们可以使用 `hg commit --amend`，如果要修改前几个的 commit 信息，可以这样：
...
如果要合并多个 commit，并且保留所有提交信息，可以这样：
...
可以使用 blame 命令查看文件中每一行的最后改动者：

```bash
> hg blame -ulc package.json
```
